...
class FlowerPack extends Component {
  constructor(props) {
    super(props);
    this.state = {
    };
    ...
    const flowerPack = color.map(elem => (
      <li key={elem.key} className="flower-pack-item">
        ...
        <div className="amount-container">
          <button className="flower-pack-button" name="button-remove" type="button" onClick={ev => this.props.changeAmount({ name: ev.target.name, key: elem.key })}>-</button>
          <input className="flower-pack-input" name="input" value={this.props.currentAmount[elem.key - 1]} onChange={ev => this.props.changeAmount({ name: ev.target.name, key: elem.key, value: (ev.target.value > 100 ? 100 : ev.target.value) })} />
          <button className="flower-pack-button" name="button-add" type="button" onClick={ev => this.props.changeAmount({ name: ev.target.name, key: elem.key })}>+</button>
        </div>
      </li>
    ));
    return (
      <ul className="flower-pack-list">{flowerPack}</ul>
    );
  }
}
...
const mapDispatchToProps = dispatch => (
  bindActionCreators({ changeAmount }, dispatch)
);

export default connect(mapStateToProps, mapDispatchToProps)(FlowerPack);
